...
const writeStockToJSON = (data) => {
  fs.writeFile("stock.json", JSON.stringify(data), (err) => {
    if (err) {
      console.log(err);
    } else {
...
//get stock from files and write to JSON
const getStockFromFiles = (filename, date, locale) => {
  let local = locale;
  ...
  worksheets.forEach((worksheet) => {
    ...
    // Remove the unused variable 'count'
    // let count = 0;
...
fileNames.forEach((filename) => {
  getStockFromFiles(filterByLocale(filename));
});

writeStockToJSON(data);
